...
class CacheEntry implements JsonSerializable, CacheItem
{
    ...
    public function removeCachedFile() : bool
    {
        if ($this->cachedFileExists()) {
            $this->manager->getContext()->logBegin(sprintf('Removing file [%s]...', $this->getTargetRelativePath()));
            unlink($this->getTargetAbsolutePath());
            $this->manager->getContext()->logEnd('REMOVED');
            return true;
        }
        return false;
    }
    ...
    public function update(CacheableItem $item) {
        ...
        if ( !$this->cachedFileExists() ) {
            $this->manager->getContext()->logBegin(sprintf('Cache file [%s] NOT FOUND!', $this->getTargetRelativePath()));
            $this->generate($item);
            $this->manager->getContext()->logEnd('');
            return true;

        } else if ( $this->last_modification_time < $item->getLastModificationTime() ) {
            $this->manager->getContext()->logBegin(sprintf('Outdated cache entry [%s] FOUND!', $item->getKey()));
            $this->generate($item);
            $this->manager->getContext()->logEnd('');
            return true;
    ...
    protected function generate(CacheableItem $item) {
        ...
        $this->manager->getContext()->logBegin(sprintf('Generating cache file [%s]...', $item->getTargetRelativePath()));
        ...
        $this->manager->getContext()->logEnd('GENERATED');
